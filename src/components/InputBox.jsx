...
export default function InputBox({ input, setInput, showUpgrade, setShowUpgrade, onSend, 
    onKeyDown, textareaRef, compact, loading, setLoading }) {
        ...
        <div className="w-full bg-[#111] border border-[#222] rounded-2xl overflow-hidden focus-within:border-[#333] transition-colors">
            ...
            <div className="flex items-center justify-between px-3 pb-2">
                ...
                <button
                    ...
                    disabled={!input.trim()}
                    className={`${loading ? "w-5 h-5" : "w-8 h-8"} cursor-pointer rounded-lg flex items-center justify-center transition-all active:scale-95 ${input.trim()
                        ? "bg-white text-black hover:bg-[#e8e8e8]"
                        : "bg-[#1a1a1a] text-[#444] cursor-not-allowed border border-[#2a2a2a]"
                        }`}
                >
                    {loading ? <OuterLoader/> : icons.ArrowUpIcon }
                </button>
            </div>
